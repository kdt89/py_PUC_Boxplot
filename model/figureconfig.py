...
class FigureConfigList:
    ...
    PLOTCONFIG_BASE_COLUMNS: str = [
        'Plot Item', 'Plot Title', 'LSL', 'USL', 'To Plot', 'Figure']
    ...
    def update(self) -> None:
        try:
            local_plot_list = pd.read_csv(
                self.LOCAL_FIGURECONFIG_FILE, index_col=None, sep=',', header=0)
        except:
            return
        ...
        plot_list_datagroups = local_plot_list.groupby(
            self.PLOTCONFIG_GROUPBY_COLUMN, sort=False)
            ...
            for row in group_data.itertuples():
                ...
                if to_plot:
                    plotconfig = PlotConfig(
                        plotname, plottitle, lowerspec, upperspec, to_plot)
